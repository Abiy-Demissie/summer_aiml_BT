...
void ballCoordinatesCallback(const geometry_msgs::Point::ConstPtr& msg)   
{
    ...
    if (msg->z <= 20){
        ...
        if(msg->y >= 0 && msg->y <= 100){
            cout<<"Turn Up msg->y is "<<msg->y<<endl;
            high_cmd_ros.mode = 1;
            high_cmd_ros.euler[1] = -0.5;
            euler_y = -0.5;

        }else if(msg->y >=100 && msg->y <=300){
            cout<<"Turn straight msg->y is "<<msg->y<<endl;
            // high_cmd_ros.mode = 1;
            // high_cmd_ros.euler[1] = 0;
            // euler_y = 0;

        }else if(msg->y >= 100 && msg->y <= 400){
            cout<<"Turn Down msg->y is "<<msg->y<<endl;
            high_cmd_ros.mode = 1;
            high_cmd_ros.euler[1] = 0.5;
            euler_y = 0.5;
        }
