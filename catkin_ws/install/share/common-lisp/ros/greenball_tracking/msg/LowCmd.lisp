...
(cl:defclass <LowCmd> (roslisp-msg-protocol:ros-message)
  ((head
   ...
   (motorCmd
    :reader motorCmd
    :initarg :motorCmd
    :type (cl:vector greenball_tracking-msg:MotorCmd)
   :initform (cl:make-array 20 :element-type 'greenball_tracking-msg:MotorCmd :initial-element (cl:make-instance 'greenball_tracking-msg:MotorCmd)))
   (bms
    :reader bms
    :initarg :bms
...
(cl:defmethod cl:initialize-instance :after ((m <LowCmd>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'LowCmd)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name greenball_tracking-msg:<LowCmd> is deprecated: use greenball_tracking-msg:LowCmd instead.")))
...
(cl:defmethod motorCmd-val ((m <LowCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader greenball_tracking-msg:motorCmd-val is deprecated.  Use greenball_tracking-msg:motorCmd instead.")
  (motorCmd m))
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <LowCmd>) istream)
  ...
  (cl:let ((vals (cl:slot-value msg 'motorCmd)))
    (cl:dotimes (i 20)
    (cl:setf (cl:aref vals i) (cl:make-instance 'greenball_tracking-msg:MotorCmd))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<LowCmd>)))
  ...
  (cl:format cl:nil "~%uint8[2] head~%uint8 levelFlag~%uint8 frameReserve~%~%uint32[2] SN~%uint32[2] version~%uint16 bandWidth~%MotorCmd[20] motorCmd~%BmsCmd bms~%uint8[40] wirelessRemote~%uint32 reserve~%~%uint32 crc~%================================================================================~%MSG: greenball_tracking/MotorCmd~%uint8 mode           # motor target mode~%float32 q            # motor target position~%float32 dq           # motor target velocity~%float32 tau          # motor target torque~%float32 Kp           # motor spring stiffness coefficient~%float32 Kd           # motor damper coefficient~%uint32[3] reserve    # motor target torque~%================================================================================~%MSG: greenball_tracking/BmsCmd~%uint8 off            # off 0xA5~%uint8[3] reserve~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'LowCmd)))
  "Returns full string definition for message of type 'LowCmd"
  (cl:format cl:nil "~%uint8[2] head~%uint8 levelFlag~%uint8 frameReserve~%~%uint32[2] SN~%uint32[2] version~%uint16 bandWidth~%MotorCmd[20] motorCmd~%BmsCmd bms~%uint8[40] wirelessRemote~%uint32 reserve~%~%uint32 crc~%================================================================================~%MSG: greenball_tracking/MotorCmd~%uint8 mode           # motor target mode~%float32 q            # motor target position~%float32 dq           # motor target velocity~%float32 tau          # motor target torque~%float32 Kp           # motor spring stiffness coefficient~%float32 Kd           # motor damper coefficient~%uint32[3] reserve    # motor target torque~%================================================================================~%MSG: greenball_tracking/BmsCmd~%uint8 off            # off 0xA5~%uint8[3] reserve~%~%"))
